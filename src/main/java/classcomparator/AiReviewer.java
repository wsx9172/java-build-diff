    ...
    public static CompletableFuture<String> aiReviewAsync(String oldContent, String newContent,
        ...
        if (isResource) {
            ...
            prompt = String.format("【任务】比较以下两个新老两个文件的内容是否等价。"
                    ...
                    + "3. 若确信存在行为差异，先输出 NO，换行后按以下格式填充{}中内容，列举功能差异（若有多条，一条差异信息一行）：\r\n"
                    + "NO\r\n"
                    + "【位置】{方法名/属性名} 【概述】：{简单描述差异}\r\n"
                    + "=== OLD FILE === %s"
            ...
            prompt = String.format("【任务】比较以下两个反编译器从 Java Class 字节码的内容是否等价，请忽略反编译器导致的结构差异。\r\n"
                    + methodHint
                    + "【核心原则】\r\n"
                    + "1. 寻找行为变化的确定性证据。可观察行为包括：修改的成员/静态变量、修改方法声明、修改方法内部逻辑。\r\n"
                    + "2. 不得因代码风格、重构方式、变量命名、表达式形式（如 if-else ↔ 三元）或无关语句顺序调整而输出 NO。\r\n"
                    + "3. 只有存在确凿证据证明可观察行为逻辑相同时，才允许输出 YES。证据不足时，必须输出 NO。\r\n"
                    + "4. 对于已确认不影响行为的差异，无需重复描述，但必须继续检查所有其他位置，不得提前终止。\r\n"
                    + "【输出规则】\r\n"
                    + "1. 严禁输出任何思考、查找或对比的过程。\r\n"
                    + "2. 若未发现行为变化的证据，只输出一个单词：YES（前后无任何其他字符）。\r\n"
                    + "3. 若确信存在行为差异，先输出 NO，换行后按以下格式填充{}中内容，列举功能差异，忽略功能相同、逻辑相同、无实际影响的差异（若有多条，一条差异信息一行）：\r\n"
                    + "NO\r\n"
                    + "【位置】{方法名/属性名} 【概述】：{简单描述差异}\r\n"
                    + "=== OLD CLASS ===\r\n"
                    + "%s\r\n"
